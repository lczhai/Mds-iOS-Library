...
#import "MDSJSVersionModel.h"
#import "YYModel.h"
#import "MDSUpdateBundlejsRequest.h"
#import <SVProgressHUD.h>
#import "NSData+bsdiff.h"
#import "MDSResourceCheck.h"
#import "MDSMediatorManager.h"
#import "MDSConfigManager.h"
#import "MDSHandlerFactory.h"
#import "MDSCheckUpdateProtocol.h"
...
typedef NS_ENUM(NSUInteger, MDSResourceVersion) {
    MDSPagesVersion = 0,
    MDSBundleVersion,
    MDSCacheVersion
};
...
typedef NS_ENUM(NSUInteger, MDSResourceCheckUpdateCode) {
    MDSResourceCheckUpdateSuccess= 0,  //查询成功
    MDSResourceCheckUpdateFail = 4001,
    MDSResourceCheckUpdateLasted = 4000
};




@interface MDSResourceManager()


@property (nonatomic, strong) MDSJSVersionModel *jsVModel;
@property (nonatomic,readwrite) MDSResourceVersion lastVerionType;

@property (nonatomic,weak)MDSUpdateBundlejsRequest * updateBundleRequest;
@property (nonatomic, copy) NSString *updateBundleJsUrl;
@end



@implementation MDSResourceManager

+ (instancetype)sharedInstance
{
    static MDSResourceManager *_instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _instance = [[MDSResourceManager alloc] init];
    });
    return _instance;
}
...
- (instancetype)init
{
    if (self = [super init]) {
        ...
        self.lastVerionType = MDSPagesVersion;
...
{
    id<MDSCheckUpdateProtocol> checkUpdateHandler = [MDSHandlerFactory handlerForProtocol:@protocol(MDSCheckUpdateProtocol)];
    if (checkUpdateHandler && [checkUpdateHandler respondsToSelector:@selector(checkUpdate)]) {
        [checkUpdateHandler checkUpdate];
        return;
    }
    
    if (![MDSConfigManager shareInstance].platform.url.bundleUpdate.length) return;
...
//    NSString * appName = currentConfig[APP_NAME_KEY]?currentConfig[APP_NAME_KEY]:@"app-benmu-health";
    ...
    /* 线上js版本 */
    MDSCheckJsVersionRequest *checkVersionApi = [[MDSCheckJsVersionRequest alloc] initWithAppName:[MDSConfigManager shareInstance].platform.appName appVersion:K_APP_VERSION jsVersion:jsVersion isDiff:isDiff];
    ...
    [checkVersionApi startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest * _Nonnull request) {
        ...
        if ([resCode intValue] == MDSResourceCheckUpdateSuccess && data) {
            
            // 有更新版本
            [weakSelf downloadRemoteJSResource:data];
            
        }
        else if([resCode intValue] == MDSResourceCheckUpdateFail){
            
            // 检测失败无对应版本

        }else if ([resCode intValue] == MDSResourceCheckUpdateLasted){
        ...
        if (NO == [[NSFileManager defaultManager] fileExistsAtPath:K_JS_PAGES_PATH] && NO == [[NSFileManager defaultManager] fileExistsAtPath:K_JS_VERSION_PATH]) {
            ...
            if (bundleVersion && currentVersion) {
                ...
                if ([lasetVersion compare:bundleVersion] == NSOrderedAscending) {
                    self.lastVerionType = MDSBundleVersion;
            ...
            if(lasetVersion && cacheVersion){
                if ([lasetVersion compare:cacheVersion] == NSOrderedAscending) {
                    self.lastVerionType = MDSCacheVersion;
...
- (void)downloadRemoteJSResource:(NSDictionary*)downloadDict
{
    ...
    /* 下载最新版本 */
    MDSUpdateBundlejsRequest *updateJsApi = [[MDSUpdateBundlejsRequest alloc] initWithDownloadJSUrl:urlString];
        ...
        if ([dict isKindOfClass:[NSDictionary class]]) {
            ...
            if (configData.length > 0) {
                ...
                if (writeSuccess) {
                    ...
                    self.mdsWidgetJs = nil;
                    [[MDSMediatorManager shareInstance] showJsResourceUpdatedAlert];
...
-(void)checkDownloadZips:(NSString*)zipPath downloadPath:(NSString*)downloadPath
{
    ...
    __weak typeof(self) weakSelf = self;
    WXLogInfo(@"校验开始");
    [MDSResourceCheck checkLocalResourceByZipPath:zipPath result:^(BOOL check, NSDictionary *info) {
        WXLogInfo(@"校验结束");
...
- (NSString *)mdsWidgetJs
{
    if (!_mdsWidgetJs) {
        ...
        else
        {
            ...
            return nil;
//            widgetJs = [NSString stringWithContentsOfURL:[MDSAppResource configJSFullURLWithPath:TK_PlatformInfo().appBoard] encoding:NSUTF8StringEncoding error:nil];
        }
        _mdsWidgetJs = widgetJs;
    }
    return _mdsWidgetJs;
}
