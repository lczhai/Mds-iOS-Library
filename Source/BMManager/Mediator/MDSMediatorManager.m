...
#import "MDSTabBarController.h"
#import "MDSWebViewController.h"
...
@interface MDSMediatorManager ()

@property (nonatomic, strong) MDSRouterModel *openVcRouterModel;    // 跳转页面时的routerModel
@property (nonatomic, weak) WXSDKInstance *openVcWeexInstance;     // 跳转页面前的weexInstance

@property (nonatomic, strong) MDSBaseViewController *jsMediator;        // 此页面作为js端的中介者，常驻内存
...
@implementation MDSMediatorManager

+ (instancetype)shareInstance
{
    static MDSMediatorManager *_instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _instance = [[MDSMediatorManager alloc] init];
    });
...
- (MDSRouterModel *)_routerModelWithURLPath:(NSString *)url navTitle:(NSString *)navTitle params:(NSDictionary *)params animateType:(NSString *)animateType navBarShow:(BOOL)navBarShow
{
    MDSRouterModel *routerModel = [[MDSRouterModel alloc] init];
    routerModel.url = url;
...
- (void)_openViewControllerWithRouterModel:(MDSRouterModel *)routerModel weexInstance:(WXSDKInstance *)weexInstance
{
    /* 初始化控制器 */
    MDSBaseViewController *controller = [[MDSBaseViewController alloc] init];
    controller.url = [MDSAppResource configJSFullURLWithPath:routerModel.url];
    ...
    {
        MDSNavigationController *navc = [[MDSNavigationController alloc] initWithRootViewController:controller];
...
- (void)loadJSMediator:(BOOL)reload
{
    ...
    MDSBaseViewController *jsMediatorVc = [[MDSBaseViewController alloc] init];
    jsMediatorVc.url = [MDSAppResource configJSFullURLWithPath:[MDSConfigManager shareInstance].platform.page.mediatorPage?:K_JS_MEDIATOR_PATH];
    [jsMediatorVc view];
...
- (UIViewController *)loadHomeViewController
{
    ...
    MDSPlatformModel *platformInfo = [MDSConfigManager shareInstance].platform;
    platformInfo.page.homePage = MDS_GetUserDefaultData(K_HomePagePath)?:platformInfo.page.homePage;
    ...
    if ([platformInfo.page.homePage isEqualToString:@"tabBar"]) {
        ...
        MDSTabBarController *tabBarController = [[MDSTabBarController alloc] init];
        tabBarController.tabBarInfo = platformInfo.tabBar;
        self.baseTabBarController = tabBarController;
        
        NSMutableArray *controllers = [NSMutableArray array];
        for (MDSTabBarItem *item in platformInfo.tabBar.list) {
            MDSBaseViewController *viewController = [[MDSBaseViewController alloc] init];
            MDSRouterModel *routerInfo = [[MDSRouterModel alloc] init];
            ...
            viewController.url = [MDSAppResource configJSFullURLWithPath:item.pagePath];
            MDSNavigationController *navc = [[MDSNavigationController alloc] initWithRootViewController:viewController];
    ...
    MDSBaseViewController *firstVc = [[MDSBaseViewController alloc] init];
    firstVc.url = [MDSAppResource configJSFullURLWithPath:platformInfo.page.homePage];
    MDSNavigationController *firstNavc = [[MDSNavigationController alloc] initWithRootViewController:firstVc];
...
{
    MDSWebViewController *webView = [[MDSWebViewController alloc] initWithRouterModel:routerInfo];
