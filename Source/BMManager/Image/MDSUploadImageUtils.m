...
@interface MDSUploadImageUtils () <UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate>

@property (nonatomic, weak) WXSDKInstance *weexInstance;
@property (nonatomic, copy) WXModuleCallback callback;
@property (nonatomic, strong) MDSUploadImageModel *imageInfo;
@property (nonatomic, assign) BOOL isLocal; /**< 通过此参数判断是否返回本地的图片地址 */

@end

@implementation MDSUploadImageUtils
...
-(void)uploadImage:(NSArray<UIImage *> *)images
{
    ...
    for (UIImage *image in images) {
        MDSUploadImageRequest *api = [[MDSUploadImageRequest alloc] initWithImage:image uploadImageModel:self.imageInfo];
    ...
    [batchRequest startWithCompletionBlockWithSuccess:^(YTKBatchRequest * _Nonnull batchRequest) {
        ...
        for (MDSUploadImageRequest *request in batchRequest.requestArray) {
            id result = [request responseObject];
            [arr4ImagesUrl addObject:result ?: @""];
        }
        if (self.callback) {
            NSDictionary *backData = [NSDictionary configCallbackDataWithResCode:MDSResCodeSuccess msg:nil data:arr4ImagesUrl];
...
- (void)cacheImages:(NSArray *)images
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (self.callback) {
                NSDictionary *backData = [NSDictionary configCallbackDataWithResCode:MDSResCodeSuccess msg:nil data:imagesPath];
                self.callback(backData);
            }
        });
    });
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        for (id item in images) {
            ...
            else if ([item isKindOfClass:[NSString class]])
            {
                ...
                if ([imgPath hasPrefix:MDS_LOCAL])
                {
                    // 拦截器
