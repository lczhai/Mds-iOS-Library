...
{
    NSInteger resCode = [[[UIApplication sharedApplication].delegate window] endEditing:YES] ? MDSResCodeSuccess : MDSResCodeError;
    if (callback) {
...
- (void)copyString:(NSString *)info callback:(WXModuleCallback)callback
{
    ...
    NSDictionary *resDic = [NSDictionary configCallbackDataWithResCode:MDSResCodeSuccess msg:@"已复制到剪切板" data:nil];
    callback(resDic);
}
...
{
    NSDictionary *resDic = [NSDictionary configCallbackDataWithResCode:MDSResCodeSuccess msg:@"获取环境信息成功" data:[WXUtility getEnvironment]];
    if (callback) {
        callback(resDic);
    }
}
...
- (void)watchNetworkStatus:(WXModuleKeepAliveCallback)callback
{
    ...
    [self.reachability setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        NSDictionary *resData = [NSDictionary configCallbackDataWithResCode:MDSResCodeSuccess msg:nil data:networkStatus];
        if (callback)
        {
