...
const static NSString * mdsNotifaction = @"mdsNotifaction";
...
@interface MDSNotifactionCenter()
{
...
@implementation MDSNotifactionCenter

+(instancetype)defaultCenter
{
    static MDSNotifactionCenter *_instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _instance = [[MDSNotifactionCenter alloc] init];
    });
    return _instance;
}
...
-(void)on:(NSString*)event callback:(WXModuleKeepAliveCallback)callback instance:(WXSDKInstance *)instance
{
    ...
    if (event.length > 0) {
        ...
        NSString * eventName = [NSString stringWithFormat:@"%@_%@",mdsNotifaction,event];
