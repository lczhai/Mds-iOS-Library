...
typedef NS_ENUM(NSUInteger,MDSNavigationItemPosition) {
    MDSNavigationItemPositionRight = 1,      /* 右边位置 */
    MDSNavigationItemPositionLeft,           /* 左边位置 */
    MDSNavigationItemPositionCenter          /* 中间位置 */
};

@interface MDSNavigatorModule ()
...
@implementation MDSNavigatorModule
...
- (void)setRightItem:(NSDictionary *)info callback:(WXModuleKeepAliveCallback)callback
{
    ...
    if (self.rightItem) {
        [self updateItemInfo:info item:self.rightItem itemPosition:MDSNavigationItemPositionRight];
        return;
    }
    
    [self createBarButton:info itemPosition:MDSNavigationItemPositionRight];
}
...
- (void)setLeftItem:(NSDictionary *)info callback:(WXModuleKeepAliveCallback)callback
{
    ...
    /** 如果已经创建过item只需更新一下文案即可 */
    if (self.leftItem) {
        [self updateItemInfo:info item:self.leftItem itemPosition:MDSNavigationItemPositionLeft];
        return;
    }
    
    [self createBarButton:info itemPosition:MDSNavigationItemPositionLeft];
}
...
- (void)setCenterItem:(NSDictionary *)info callback:(WXModuleKeepAliveCallback)callback
{
    ...
    /** 如果已经创建过item只需更新一下文案即可 */
    if (self.centerItem) {
        [self updateItemInfo:info item:self.centerItem itemPosition:MDSNavigationItemPositionCenter];
        return;
    }
    
    [self createBarButton:info itemPosition:MDSNavigationItemPositionCenter];
}
...
- (void)setNavItemWithView:(UIView *)view itemPosition:(MDSNavigationItemPosition)itemPosition
{
    if (!view) {
...
- (void)createBarButton:(NSDictionary *)param itemPosition:(MDSNavigationItemPosition)itemPosition
{
...
- (void)updateItemInfo:(NSDictionary *)param item:(UIButton *)button itemPosition:(MDSNavigationItemPosition)itemPosition
{
    if (param[@"text"]) {
        NSString *title = param[@"text"];
