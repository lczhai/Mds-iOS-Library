...
@interface MDSBaseViewController () <UIScrollViewDelegate, UIWebViewDelegate>
...
@property (nonatomic, assign) MDSControllerState controllerState;

@end

@implementation MDSBaseViewController
...
{
    /* 标示页面首次打开 */
    self.controllerState = MDSControllerStateOpen;
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    /* 设置状态栏样式 */
    [self mdsSetStatusBarStyle];

    /* 保存当前栈顶的 WXSDKInstance 与 ViewController */
    [[MDSMediatorManager shareInstance] setCurrentWXInstance:self.instance];
    [[MDSMediatorManager shareInstance] setCurrentViewController:self];
    
    // 通知js页面生命周期
    if (self.controllerState != MDSControllerStateOpen) [MDSGlobalEventManager sendViewLifeCycleEventWithInstance:_instance event:MDSViewWillAppear controllerState:self.controllerState];
}
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    // 通知js页面生命周期
    if (self.controllerState != MDSControllerStateOpen) [MDSGlobalEventManager sendViewLifeCycleEventWithInstance:_instance event:MDSViewDidAppear controllerState:self.controllerState];
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [MDSGlobalEventManager sendViewLifeCycleEventWithInstance:_instance event:MDSViewWillDisappear controllerState:self.controllerState];
}
...
- (void)viewDidDisappear:(BOOL)animated
{
    ...
    [MDSGlobalEventManager sendViewLifeCycleEventWithInstance:_instance event:MDSViewDidDisappear controllerState:self.controllerState];
    self.controllerState = MDSControllerStateBack;
...
{
    for (MDSCommonRequest *request in self.arr4Request) {
...
- (void)_renderWithURL:(NSURL *)sourceURL
{
    ...
    [_instance renderWithURL:[NSURL URLWithString:newURL] options:@{@"bundleUrl":sourceURL.absoluteString, @"eros":[MDSConfigManager shareInstance].envInfo} data:nil];
    
    __weak typeof(self) weakSelf = self;
    _instance.onCreate = ^(UIView *view) {
        
        if (weakSelf.controllerState != MDSControllerStateRefresh) {
    ...
    _instance.renderFinish = ^(UIView *view) {
        ...
        if (![weakSelf currentVcIs:TK_PlatformInfo().page.mediatorPage]) {
            [[MDSMediatorManager shareInstance] setCurrentWXInstance:weakSelf.instance];
        }
        // 通知 js 页面生命周期方法
        [MDSGlobalEventManager sendViewLifeCycleEventWithInstance:weakSelf.instance event:MDSViewWillAppear controllerState:weakSelf.controllerState];
        [MDSGlobalEventManager sendViewLifeCycleEventWithInstance:weakSelf.instance event:MDSViewDidAppear controllerState:weakSelf.controllerState];
...
{
    /* 标示页面状态刷新 */
    self.controllerState = MDSControllerStateRefresh;
