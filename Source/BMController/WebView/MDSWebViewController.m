...
@interface MDSWebViewController () <UIWebViewDelegate, JSExport>
{
    BOOL _showProgress;
}
...
@implementation MDSWebViewController

- (void)dealloc
{
    NSLog(@"dealloc >>>>>>>>>>>>> MDSWebViewController");
    if (_jsContext) {
        _jsContext[@"MDSNative"] = nil;
        _jsContext = nil;
    }
}
...
- (void)reloadURL
{
    ...
    if ([url.scheme isEqualToString:MDS_LOCAL])
    {
        // 拦截器
...
- (void)injectionJsMethod
{
    ...
    self.jsContext[@"MDSNative"] = mdsNative;
}
