...
#define MDS_FONT_DOWNLOAD_DIR [[WXUtility libraryDirectory] stringByAppendingPathComponent:[NSString stringWithFormat:@"mds-iconfont"]]


const char * fontStorageKey = "_fontStorage";

@interface MDSAddRuleManager()

@end


@implementation MDSAddRuleManager
...
//    @property (nonatomic, strong) WXThreadSafeMutableDictionary *fontStorage;
    ...
    if ([type isEqualToString:@"fontFace"] && [rule[@"src"] isKindOfClass:[NSString class]]) {
        if (rule[@"src"] && rule[@"fontFamily"] && ![WXUtility isBlankString:rule[@"src"]]) {
            ...
            //创建 mds-iconfont 目录
            [MDSAddRuleManager createIconfontDownloadPath];
            
            if ([fontURL.scheme isEqualToString:MDS_LOCAL]) {
                // 拦截器
            ...
            // remote font file
            NSString *fontfile = [NSString stringWithFormat:@"%@/%@",MDS_FONT_DOWNLOAD_DIR,[WXUtility md5:[fontURL absoluteString]]];
            ...
            [WXUtility getIconfont:fontURL completion:^(NSURL * _Nonnull url, NSError * _Nullable error) {
                if (!error && url) {
                    ...
                    [MDSAddRuleManager copyIconfontDirectory:url];
...
{
    if (![WXUtility isFileExist:MDS_FONT_DOWNLOAD_DIR]) {
        NSError * error = nil;
        BOOL createSuccess = [[NSFileManager defaultManager] createDirectoryAtPath:MDS_FONT_DOWNLOAD_DIR withIntermediateDirectories:YES attributes:nil error:&error];
        if (createSuccess) {
            WXLogInfo(@"创建iconfont文件夹成功");
            [[self class] addSkipBackupAttributeToItemAtPath:MDS_FONT_DOWNLOAD_DIR];
        }
    }
}
...
//    file:///Users/doujingxuan/Library/Developer/CoreSimulator/Devices/84BEE3B5-47E3-4862-BA97-CBBE513041A8/data/Containers/Data/Application/BC223D6C-41A9-4A96-9B68-48166A53A16B/Library/Caches/wxdownload/2f5217a9967c7f3cdccad3530f0a645f
    ...
    if ([WXUtility isFileExist:cacheIconfontPath]) {
        ...
        NSString * newPath = [MDS_FONT_DOWNLOAD_DIR stringByAppendingPathComponent:pathComponent];
