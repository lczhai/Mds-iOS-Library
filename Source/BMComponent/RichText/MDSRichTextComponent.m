//
//  MDSRichTextComponent.m
//  Pods
//
//  Created by jony on 2018/4/12.
...
@interface MDSRichTextComponent()
...
@implementation MDSRichTextComponent
{
...
- (NSAttributedString *)buildAttributeStringWithSpan:(MDSSpanComponent *)span
{
...
- (void)updateRichText
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        /** 遍历所有子组件，拿到所有mdsspan组件，构建完整的富文本 */
        for (WXComponent *subcomponent in self.subcomponents) {
            
            MDSSpanComponent *span = [self findSpanComponent:subcomponent];
            if (!span) continue;
...
/** 递归遍历子组件拿返回 mdsspan 组件 */
- (MDSSpanComponent *)findSpanComponent:(WXComponent *)component
{
    if (component) {
        if ([component isKindOfClass:[MDSSpanComponent class]]) {
            return (MDSSpanComponent *)component;
        }
